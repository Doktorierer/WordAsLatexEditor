...
> Overleaf is good, but the revision feature is not at par with MS Word. 

> Supervisor: Send me the word-file!
...
- LaTeX-environments are added as pictures in MS Word: Figure, table, math environments, such as equation, align etc., and LaTeX clutter can be hidden or greyed out, by using the formatting/hiding features of MS Word 
- The prettified .doc (which is in fact a LaTeX file) can be edited further in MS Word, or copied back to your favourite LaTeX editor (just Ctrl-A, Ctrl-C).
...
The macros in the MS Word template let you (all functionality through buttons on a new `LaTeX` tab in the Word ribbon):

- Preview LaTeX-environments (in current version, all environement are iterated): `equation`, `align`, `table`, `figure`, etc. (see full list below)
- Format some common LaTeX-markup to gray or white, and text inside `label`, `(eq)ref`, `cite(tp)` to blue (see full list below)
- Format LaTeX-section markups into Word headings (see full list below). With this, navigation and section-folding is possible by using MS Word's navigation pane and the folding features of headings 
- Hide LaTeX-markup (using the hidden-text feature of Word). See full list of markup below.
- Run pdfLaTeX for the whole Word document and display the resulting .pdf
- Delete all LaTeX-markup
- A simple unicode <-> TeX-conversion of very few charapcters/markup items (see full list further below): 
...
2. For a nice viewing in MS Word:
	 ...
	 * LaTeX-comments must be written with two %% to be seen after the special formatting commands (hiding). Hence, comments with single % are comments in Word visible for co-authors
3. Run the corresponding preview command (accessible through the new LaTeX-Ribbon): table, equation, figure, align etc. as pictures
...
- Pictures in MS Word can in the so-called `canvas` layer, then they are called `shapes` in MS Word
- Pcitures can also be in the in the so-called `document` layer (inline with text), then they are called `inline shapes`

Note: If the generated shapes are converted to inline shapes, then a control character is added by Word into the text neaerby.
...
- The LaTeX-Ribbon-command `RunLaTeX` deletes also all inline shapes in order to be able to to run `pdftex` on the document. 


### Bibliography

There is no preview of bibliography. This can be done in a minute manually. 
...
The following LaTeX-items gets replaced by UTF characters (4-digit numbers):
...
- Only LaTeX files which consists of ASCII characters only work surely; UTF files may work 
